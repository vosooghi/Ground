...
namespace Ground.Core.ApplicationServices.Commands
{
    ...
    public class CommandDispatcher : ICommandDispatcher
    {
        ...
        public async Task<CommandResult> Send<TCommand>(TCommand command) where TCommand : class, ICommand
        {
            ...
            {
                _stopwatch.Stop();
                _logger.LogInformation(GroundEventId.PerformanceMeasurement, "Processing the {CommandType} command tooks {Millisecconds} Millisecconds", command.GetType(), _stopwatch.ElapsedMilliseconds);
            }
